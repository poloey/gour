...
add_action('wp_head','shopkeeper_catalogmode');
function shopkeeper_catalogmode() {

	global $shopkeeper_theme_options;

	if (isset($shopkeeper_theme_options['catalog_mode']) && $shopkeeper_theme_options['catalog_mode'] == 1):
		$cm = 1;
	else:
		$cm = 0;
	endif; 
?>
    <script type="text/javascript">
        var shopkeeper_catalogMode = <?php echo $cm; ?>;
    </script>
<?php
}
...
function shopkeeper_scripts_header_priority_0() {
	...
	if ( (isset($shopkeeper_theme_options['smooth_transition_between_pages'])) && ($shopkeeper_theme_options['smooth_transition_between_pages'] == "1" ) ) {
		...
		wp_enqueue_script('shopkeeper-page-in-out', get_template_directory_uri() . '/js/components/page-in-out.js', array('shopkeeper-nprogress'), getbowtied_theme_version(), FALSE);
	}
...
function shopkeeper_scripts() {
	...
	if( is_rtl() )
	{
		wp_enqueue_script('shopkeeper-rtl-js', get_template_directory_uri() . '/js/rtl.js', array('jquery'), getbowtied_theme_version(), TRUE);
	}
